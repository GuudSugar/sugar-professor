  ...
  const loadMedia = (url, containerId) => {
      fetch(url)
          ...
          .then(data => {
              ...
              const carouselList = mediaContainer.querySelector('.carousel__list');
              data.forEach((media) => {
              ...
              setupFullScreenView(carouselList);
          })
          .catch(error => console.error('Error fetching media:', error));
  };

  loadMedia('newspaper.json', 'newspaper');
  loadMedia('infographics.json', 'infographics');
  loadMedia('video.json', 'video');
});

const setupCarouselNavigation = (carouselContainer, totalItems) => {
  const prevButton = carouselContainer.querySelector('.left-arrow');
  const nextButton = carouselContainer.querySelector('.right-arrow');
...
const setupFullScreenView = (carouselList) => {
  ...
  const setupImageFullScreen = () => {
      carouselList.querySelectorAll('.carousel__item img').forEach((img, index) => {
          img.addEventListener('click', function() {
              mediaItems = Array.from(carouselList.querySelectorAll('.carousel__item img'));
              currentIndex = index;
  ...
  const setupVideoFullScreen = () => {
      carouselList.querySelectorAll('.carousel__item video').forEach((video, index) => {
          const playButton = video.parentElement.querySelector('.play-button');
          const triggerFullScreen = () => {
              mediaItems = Array.from(carouselList.querySelectorAll('.carousel__item video'));
              currentIndex = index;
